...
<div class="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-lg">
    ...
    <form action="{{ route('admin.books.update', $book->slug) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3" class="w-full p-3 border border-gray-300 rounded-md">{{ old('description', $book->description) }}</textarea>
        </div>

        <!-- Author -->
        <div class="mb-4">
            <label for="author" class="block text-gray-700">Penulis</label>
            <input type="text" name="author" id="author" class="w-full p-3 border border-gray-300 rounded-md" value="{{ old('author', $book->author) }}">
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="category_id" id="category" class="w-full p-3 border border-gray-300 rounded-md" required>
                ...
                <option value="{{ $category->id }}" {{ $category->id == $book->id_category ? 'selected' : '' }}>
                    {{ $category->name }}
                </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="publisher" id="publisher" class="w-full p-3 border border-gray-300 rounded-md" value="{{ old('publisher', $book->publisher) }}">
        </div>

        <!-- Publish Date -->
        <div class="mb-4">
            <label for="publish_date" class="block text-gray-700">Tanggal Terbit</label>
            <input type="date" name="publish_date" id="publish_date" class="w-full p-3 border border-gray-300 rounded-md" value="{{ old('publish_date', $book->publish_date ? \Carbon\Carbon::parse($book->publish_date)->format('Y-m-d') : '') }}">
        </div>

        <!-- ISBN -->
        <div class="mb-4">
            <label for="isbn" class="block text-gray-700">ISBN</label>
            <input type="text" name="isbn" id="isbn" class="w-full p-3 border border-gray-300 rounded-md" value="{{ old('isbn', $book->isbn) }}">
        </div>

        <!-- Pages -->
        <div class="mb-4">
            <label for="pages" class="block text-gray-700">Jumlah Halaman</label>
            <input type="number" name="pages" id="pages" class="w-full p-3 border border-gray-300 rounded-md" value="{{ old('pages', $book->pages) }}">
        </div>

        <!-- Language -->
        <div class="mb-4">
            <label for="language" class="block text-gray-700">Bahasa</label>
            <input type="text" name="language" id="language" class="w-full p-3 border border-gray-300 rounded-md" value="{{ old('language', $book->language) }}">
        </div>
        ...
        <div class="mb-4 grid grid-cols-3 gap-2">
            <div>
                ...
                <input type="number" step="0.01" name="length" id="length" class="w-full p-3 border border-gray-300 rounded-md" value="{{ old('length', $book->length) }}">
            </div>
            <div>
                <label for="width" class="block text-gray-700">Lebar (cm)</label>
                <input type="number" step="0.01" name="width" id="width" class="w-full p-3 border border-gray-300 rounded-md" value="{{ old('width', $book->width) }}">
            </div>
            <div>
                <label for="weight" class="block text-gray-700">Berat (gram)</label>
                <input type="number" step="0.01" name="weight" id="weight" class="w-full p-3 border border-gray-300 rounded-md" value="{{ old('weight', $book->weight) }}">
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            @if($book->image)
            <img src="{{ asset('storage/' . $book->image) }}" alt="Book Image" class="mb-2 w-32 h-auto object-cover rounded">
            @endif
